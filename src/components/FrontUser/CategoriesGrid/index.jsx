...
export default function CategoriesGrid(
  ...
  const [filters, setFilters] = useState({
    ...queryParams,
    printAreaIds: queryParams?.printAreaId?.split(',').map(value => +value) || [],
    techniqueIds: queryParams?.techniqueId?.split(',').map(value => +value) || [],
  });
  const gridConfig = {
    ...
    getDataFunc: (params, successCallback, failureCallback) => {
      const { categoryId,  collectionId, printAreaIds = [], techniqueIds = [], sortByValue, ...restParams} = params || {};
      const {sortBy, sortDirection} = sortByItems.find(item => item.key === sortByValue)?.params || {}
  ...
  const filtersCount = useMemo(() => Object.keys(cui.removeEmpty(filterOptionsValues))
    .filter(key => typeof filterOptionsValues[key] === 'object' ? filterOptionsValues[key].length : !!filterOptionsValues[key])
      .length
    , [filterOptionsValues]);
    ...
    if (selectedCollection?.collectionName) {
      ...
      categories.forEach((category) => {
        ...
        if (category.categoryId === categoryId) {
          ...
          selectedCategory = category.child.find(subCategory => subCategory.categoryId === categoryId);
          if (selectedCategory) {
            selectedCategories.push(category);
      ...
      onFilterBreadCrumbs(selectedCategories.map(((category, index) => (
        index === (selectedCategories.length - 1) ? ({
          breadcrumbName: category?.categoryName,
        }) : ({
          breadcrumbName: category?.categoryName,
          path: ROUTERS.FRONT_USER_ALL_PRODUCTS + `/category/${category.categorySlug}/${category.categoryId}`,
        })
      ))))
    }
  ...
  const isAllowGridSelection = SellerSystemService.getSystemConfigValue(systemConfigs, SYSTEM_CONFIG_VALUE.PUBLIC_VIEW_PRODUCT_DETAIL) !== "false";
  return (
    <Row gutter={isMobile ? [0, 0] : [50, 0]} className={"categories-grid__wrapper"}>
      {
        !isMobile && (
          <Col span={ isMobile ? 24 : 6 } className="categories-grid__filters-sibar">
            {
              !collectionId && (
                <CategoriesFilters
                  categoryId={categoryId}
      ...
      }
      {
        isMobile && (
          <Row gutter={[0, 16]} className="categories-grid__mobile-filters-box">
            ...
            <Col span={24} className="categories-grid__options-filter">
              <span>{`${totalCount} products`}</span>
              <span className="cursor-pointer" onClick={() => setOpenFilters(true)}>
                <span>Filters</span>
                { !!filtersCount && <span className="categories-grid__filters-count">{filtersCount}</span>}
              </span>
            </Col>
          </Row>
        )
      ...
      <Col span={ isMobile ? 24 : 18 } className="categories-grid__product-list">
        <Row gutter={isMobile ? [0, 16] : [32, 32]}>
          {
            !categoryId && !collectionId && (
              <Col span={24} style={{ display: categories.length ? 'block' : 'none'}}>
                <TopCategoriesBox
